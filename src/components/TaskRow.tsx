...
import { TYPE_LABELS, boardColor, boardName, formatDisplayDate, formatDisplayDateTime } from '../utils/helpers'
...
export default function TaskRow({
  ...
  const { data, softDeleteTask, toggleFavorite, togglePin, startTimer, reopenTask, duplicateTask } = useApp()
    ...
    <div className={`queue-row task-row ${task.isPinned ? 'is-pinned' : ''}`}>
      ...
      <div className="queue-main" onDoubleClick={() => onEdit(task)}>
        ...
        <div className="queue-meta">
          ...
          {task.workDate && <span>{formatDisplayDate(task.workDate)}</span>}
          ...
          {showDoneMeta && task.gitCommit && <span>#{task.gitCommit.slice(0, 10)}</span>}
        </div>
      ...
      <div className="row-actions">
        ...
        {showDoneMeta && task.completedAt && (
          <span className="done-time">{formatDisplayDateTime(task.completedAt)}</span>
        )}
      </div>
    </div>
  )
